...
def passes_base_filters(f: PairFeatures) -> Tuple[bool, str]:
    # Sector-aware filters: tighter for cross-sector, slightly looser for same-sector / infra.
    ...
    recent_adf_ceiling = 0.12

    if f.pair_type == "same_sector":
        corr_floor = 0.65
        roll_floor = 0.60
        adf_ceiling = 0.12
        recent_adf_ceiling = 0.15
    ...
    if pd.isna(f.recent_adf_p) or f.recent_adf_p > recent_adf_ceiling:
...
class StatArbMLBot:
    ...
    def evaluate_new_entries(self, market_data: Dict[str, pd.DataFrame], btc_df: pd.DataFrame):
        ...
        for pair in self.pairs:
            ...
            current_bar_ts = pd.Timestamp(f.timestamp)
            if last_pair_bar is not None:
                try:
                    bars_since_last = int((current_bar_ts - pd.Timestamp(last_pair_bar)).total_seconds() / 3600)
                except Exception:
                    bars_since_last = 0
                if bars_since_last < CONFIG["cooldown_bars_same_pair"]:
                    continue
        ...
        while True:
            try:
                ...
                if last_processed_bar is None or current_bar > last_processed_bar:
                    ...
                    self.run_cycle()
                    print(f"[CYCLE] processed new bar {current_bar} | equity=${self.demo_equity:.2f} | open={len(self.open_positions())}")
                else:
                    print(f"[WAIT] no new 1h bar yet | last_bar={current_bar} | equity=${self.demo_equity:.2f} | open={len(self.open_positions())}")
